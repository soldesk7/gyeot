...
public record EgytListInfoResponse(Header header, Body body) {
     ...
     *
     * 이 타입만 record가 아닌 이유: @JacksonXmlElementWrapper가 record에서 동작하지 않는 Jackson 버그(jackson-dataformat-xml#517) 때문
     * record로 두면 "Could not find creator property with name 'items'" 오류가 난다. 
     * record는 "생성자 파라미터 이름 = 필드 이름"인데 @JacksonXmlProperty가 그 이름을 item으로 바꿔버리므로 items라는 이름을 찾는 wrapper와 어긋나기 때문. 
    ...
    public static final class Body {
        ...
        /**
         * 조회 결과가 0건이면 null이 될 수 있다 — 호출부에서 방어. 이 메서드는 Jackson이 쓰는 게 아니라 우리 코드가 값을 꺼낼 때 쓰는 접근자이다. 
         * 이름을 getItems()가 아니라 items()로 지은 이유: 
         * 이 파일의 나머지가 전부 record라 호출부에서 response.body().items()처럼 일관된 모양이 되게 하기 위함. 
