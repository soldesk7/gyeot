...
public class RecognitionService {
    ...
    public RecognitionResponse recognize(byte[] imageBytes, String mimeType) {
        try {
            ...
            return new RecognitionResponse(category, result.confidence(), result.visibleSigns(), isLowConfidence(category, result.confidence()));
        } catch (Exception e) {
    ...
    /**
     * 인식 결과를 확정된 사실처럼 보여줘도 되는지 판정한다. 범주를 못 고른 경우(UNKNOWN)는
     * 확신도와 무관하게 저확신으로 본다 — 보여줄 범주 자체가 없기 때문이다.
     */
    private static boolean isLowConfidence(RecognitionCategory category, double confidence) {
        return category == RecognitionCategory.UNKNOWN || confidence < LOW_CONFIDENCE_THRESHOLD;
    }
}